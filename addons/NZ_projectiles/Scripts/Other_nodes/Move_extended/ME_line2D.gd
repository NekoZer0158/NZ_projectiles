...
@export var queue_free_if_cant_move_on_line : bool = false ## If the line isn't valid than the projectile will queue_free().
@export var find_line_through_parent_node : bool = true
...
func _ready() -> void:
	...
	if _can_find_line(parent_node):
		if find_line_through_parent_node:
			set_line(parent_node.get_node(line_path),parent_node)
		else:
			set_line(get_node(line_path),parent_node)
	else:
...
func _can_find_line(parent_node : Projectile) -> bool:
	return (find_line_through_parent_node and parent_node.has_node(line_path)) or (!find_line_through_parent_node and has_node(line_path))
