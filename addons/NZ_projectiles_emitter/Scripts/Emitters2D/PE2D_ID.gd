...
func emit(type:int=0) -> void:
	if can_emit:
		var projectile_instance : Projectile = get_projectile_by_method(default_projectile_id)
		_add_projectile_instance_to_the_scene(projectile_instance,type)

func get_projectile_by_method(id:String) -> Projectile:
...
func emit_by_id(id:String,type:int=0) -> void:
	if can_emit:
		var projectile_instance : Projectile = get_projectile_by_method(id)
		if projectile_instance != null:
			_add_projectile_instance_to_the_scene(projectile_instance,type)
